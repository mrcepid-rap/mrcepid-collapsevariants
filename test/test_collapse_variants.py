...
tmp_path = Path(test_dir / 'test_data/tmp_path')
...
# Test input (for working out the flow of information)
filtering_expression = 'PARSED_CSQ=="PTV" & LOFTEE=="HC" & MAF < 0.001'
gene_list_path = gene_symbol_path
snp_list_path = None
...
def test_snp_and_gene_masks(tmp_path: Path, filtering_expression: str, gene_list_path: Path,
    ...
    # Generate the genotype matrices for out bgen files
    genotype_index = {}
    for bgen_prefix, variant_list in snp_list_generator.genes.items():
    ...
    if snp_list_path:
        ...
        # Filter for files ending with '.bgen'
        bgen_files = [str(path) for path in output_files if str(path).endswith('.bgen')]
        assert len(bgen_files) == 3

        # Filter for files ending with '.bgen.bgi'
        bgen_bgi_files = [str(path) for path in output_files if str(path).endswith('.bgen.bgi')]
        assert len(bgen_bgi_files) == 3

        # Filter for files ending with '.sample'
        sample_files = [str(path) for path in output_files if str(path).endswith('.sample')]
        assert len(sample_files) == 3

        # Filter for files ending with '.annotationFile.txt'
        annotation_files = [str(path) for path in output_files if str(path).endswith('.annotationFile.txt')]
        assert len(annotation_files) == 3

        # Filter for files ending with '.maskfile.txt'
        mask_files = [str(path) for path in output_files if str(path).endswith('.maskfile.txt')]
        assert len(mask_files) == 3

        # Filter for files ending with '.setListFile.txt'
        set_list_files = [str(path) for path in output_files if str(path).endswith('.setListFile.txt')]
        assert len(set_list_files) == 3

        # Filter for files ending with '.groupFile.txt'
        group_files = [str(path) for path in output_files if str(path).endswith('.groupFile.txt')]
        assert len(group_files) == 3

        # Filter for files ending with '.samples_table.tsv'
        samples_table_files = [str(path) for path in output_files if str(path).endswith('.samples_table.tsv')]
        assert len(samples_table_files) == 3

        # Filter for files ending with '.variants_table.tsv'
        variants_table_files = [str(path) for path in output_files if str(path).endswith('.variants_table.tsv')]
        assert len(variants_table_files) == 3


@pytest.mark.parametrize(
    "testing_directory",
    [os.getcwd()]  # Automatically sets the current directory
)
def test_check_file_consistency(testing_directory):
    """
    ...
    """
    test_dir = Path(testing_directory)
    # check to make sure the samples are consistent
    snp_samples = pd.read_csv(Path(test_dir / 'testing_output.SNP.STAAR.samples_table.tsv'), sep='\t')
    gene_samples = pd.read_csv(Path(test_dir / 'testing_output.GENE.STAAR.samples_table.tsv'), sep='\t')

    assert snp_samples['sampID'].any() == gene_samples['sampID'].any()

    bolt_sample = pd.read_csv(Path(test_dir / 'testing_output.chr1_chunk1.BOLT.sample'), sep=' ')
    starr_sample = pd.read_csv(Path(test_dir / 'testing_output.chr1_chunk1.STAAR.samples_table.tsv'), sep='\t')

    assert bolt_sample['ID_1'].any() == starr_sample['sampID'].any()

    staar_sample_1 = pd.read_csv(Path(test_dir / 'testing_output.chr1_chunk1.STAAR.samples_table.tsv'), sep='\t')
    staar_sample_2 = pd.read_csv(Path(test_dir / 'testing_output.chr1_chunk2.STAAR.samples_table.tsv'), sep='\t')

    assert staar_sample_1['sampID'].any() == staar_sample_2['sampID'].any()

    # check that the variants tables are correct
    snp_variants = pd.read_csv(Path(test_dir / 'testing_output.SNP.STAAR.variants_table.tsv'), sep='\t')
    assert snp_variants['POS'].all() < 90000000

    staar_variants_1 = pd.read_csv(Path(test_dir / 'testing_output.chr1_chunk1.STAAR.variants_table.tsv'), sep='\t')
    ...
    staar_variants_2 = pd.read_csv(Path(test_dir / 'testing_output.chr1_chunk2.STAAR.variants_table.tsv'), sep='\t')
