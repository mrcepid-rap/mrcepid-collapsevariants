...
**Note:** One can also filter based on
raw [VEP consequence](https://www.ensembl.org/info/genome/variation/prediction/predicted_data.html#consequences)
...
Variants
listed can be from multiple chromosomes or a single gene. variant IDs **MUST** match the format found in the VEP
annotation
...
mask.
The relevant parameter to specify the gene list is `genelist`. As with the SNP list input we create a single 'GENE'
which here has the ID ENST99999999999.
Many considerations for the SNP list input also apply to gene list input (e.g. maximum number of underlying variants
etc.).
Gene IDs are checked against the HGNC gene symbols reported in the VEP annotation files, example input would look like
this:
...
final tab-delimited output files provided for each tool. These columns are derived by splitting `file_prefix` on "-".
For
example, if `file_prefix` is "PTV-HC", where "PTV" is the type of variant and "HC" is the LoFTee setting used to filter
PTVs,
two columns in the final associationtesting output will be PTV and HC. As many "dashes" as desired can be included to
derive
multiple columns of information (e.g. "MISSENSE-CADD_25-REVEL_07-MAF_01-POPALL" will result in 5 additional columns in
the
...
includes
either the keywords "MAF" or "AC" (e.g. "HC_PTV-MAF_01"). This will result in associationtesting naming additional
columns
...
with
one set of files for each chromosome (please see below for more detailed descriptions of each file):

* <file_prefix>.<chr>.BOLT.bgen – BOLT-ready .bgen file of per-gene 'genotypes'.
* <file_prefix>.<chr>.BOLT.sample – BOLT-ready bgen sample file of per-gene 'genotypes'.
* <file_prefix>.<chr>.SAIGE.groupFile.txt – TSV file of variants assigned to each gene.
* <file_prefix>.<chr>.REGENIE.annotationFile.txt – TSV of variant assigned to gene and mask.
* <file_prefix>.<chr>.REGENIE.setListFile.txt – TSV of variants assigned to genes with coordinate information
* <file_prefix>.<chr>.REGENIE.maskefile.txt – TSV of masks in the given output
...
If providing a SNP list, the only difference for the above is that a single set of files with the prefix <file_prefix>
.SNP
...
files
with the prefix <file_prefix>.GENE is generated. The log file is again identical except for a section in the beginning
...
* <file_prefix>.<chr>.REGENIE.maskfile.txt – TSV of masks in the given output
...
* <file_prefix>.<chr>.STAAR.variants_table.tsv – TSV of variant assigned to columns
...
* <file_prefix>.<chr>.STAAR.samples_table.tsv - TSV of sample IDs assigned to rows
